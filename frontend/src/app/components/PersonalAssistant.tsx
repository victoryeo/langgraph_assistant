...
export default function PersonalAssistant({ onBack }: AssistantProps) {
  ...
  const handleFetchTasks = async () => {
    try {
      ...
      setTaskError(null);
      const fetchedTasks = await fetchTasks('personal');
  ...
  const handleCompleteTask = async (taskId: string) => {
    try {
      ...
      const updatedTask = await completeTask('personal', taskId);
  ...
  const handleDeleteTask = async (taskId: string, event: React.MouseEvent) => {
    ...
    try {
      ...
      const result = await deleteTask('personal', taskId);
        ...
        {tasks.length === 0 ? (
          <div className="h-full flex items-center justify-center">
            <div className="text-center text-gray-400">
              <p className="text-sm mt-1">Ask me anything about personal</p>
            </div>
          </div>
        ) : (
          <div className="h-full flex items-center justify-center">
